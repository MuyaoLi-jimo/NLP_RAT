...
def rat(client:OpenAI, system_prompt:str, question:str):
    ...
    for i, p in enumerate(draft_paragraphs):
        ...
        if query['type'] == "天气":
            ...
            print(f"{datetime.now()} [INFO] 获取到天气信息：\n{weather_info}")
            ...
            if len(ref_trees) == 0:
                ...
                r_content = routes_to_content(ref_routes, baike_jsons)
                # 加入百科的简介部分
                ref_content = ""
                for baike in baike_jsons:
                    ref_content += json.dumps({"词条":baike['title'],"简介":baike['content']}, indent=4, ensure_ascii=False) + '\n'
                ref_content += r_content
